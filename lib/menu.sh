        ...
        3)
            if ! directory_empty $database_path; then
                ...
                select db in "${databases[@]}"; do
                    if [[ -n $db ]]; then
                        if ! directory_empty $db; then
                            echo -e "\e[31mWarning:\e[0m There are tables in this database are you sure you want to drop it?"
                            read -p "Press (Y/y) to continue: " choice
                            case "$choice" in
                            [yY])
                                dropDB "$db"
                                sleep 3
                ...
                *)
                ...
                select table in "${tables[@]}"; do
                    if [[ -n $table ]]; then
                        ...
                        echo "${fields[@]}"
                        echo -e "The primary key field is: \e[32m${fields[$pk_field_number - 1]}\e[0m"
        ...
        1) #-------------------------- Select all data ----------------------#
            if ! file_empty "$table"; then
                ...
                *)
                ...
                select_all_table_data "$table"
                PS3="$dbname> "
                display_table_menu "$dbname"
            fi
            ;;
        2) #-------------------------- Select column data ----------------------#
            if ! file_empty "$table"; then
                echo -e "\e[36mThere is no data in table "$table"\e[0m"
                read -p "Press (C/c) to continue: " choice
                case "$choice" in
                ...
                *)
                ...
                select field in "${fields[@]}"; do
                    if [[ -n $field ]]; then
                        ...
                        select_column_data "$table" "$REPLY"
                        PS3="$dbname> "
                        display_table_menu "$dbname"
                    else
                        echo -e "\e[31mWarning:\e[0m invalid choice"
                ...
                *)
                ...
                select field in "${fields[@]}"; do
                    if [[ -n $field ]]; then
                        ...
                        if ! select_row_data "$table" "$REPLY" "$column_value"; then
        ...
        1) #-------------------------- Delete all data ----------------------#
            if ! file_empty "$table"; then
                ...
                *)
                ...
                delete_all_table_data "$table"
                PS3="$dbname> "
                display_table_menu "$dbname"
            fi
            ;;
        2) #-------------------------- Delete column data ----------------------#
            if ! file_empty "$table"; then
                echo -e "\e[36mThere is no data in table "$table"\e[0m"
                read -p "Press (C/c) to continue: " choice
                case "$choice" in
                ...
                *)
                ...
                pk_field_number=$(get_primary_key_number "$table")
                local not_null_fields=$(get_not_null_columns "$table")
                    ...
                    if [[ -n $field ]]; then
                        ...
                        while [ $valid -eq 0 ]; do
                            if (($REPLY == $pk_field_number)); then
                                echo -e "\e[31mWarning:\e[0m Column "$field" is the primary key and can't be deleted"
                                echo ""
                                display_delete_from_table_menu "$table" "$dbname"
                            fi

                            if [[ "${not_null_fields[@]}" =~ "$REPLY" ]]; then
                                echo -e "\e[31mWarning:\e[0m Column "$field" has a not null constrains and can't be deleted"
                                echo ""
                                display_delete_from_table_menu "$table" "$dbname"
                            fi
                            valid=1
                        ...
                        delete_column_data "$table" "$REPLY"
                        PS3="$dbname> "
                        display_table_menu "$dbname"
                    else
                        echo -e "\e[31mWarning:\e[0m invalid choice"
                ...
                *)
                ...
                select field in "${fields[@]}"; do
                    if [[ -n $field ]]; then
                        valid=0
                        while [ $valid -eq 0 ]; do
                            read -p "Please enter the value that you want to delete it's rows: " column_value
                        ...
                        if ! delete_row_data "$table" "$REPLY" "$column_value"; then
                            echo -e "\e[31mWarning:\e[0m No data found"
                            display_delete_from_table_menu "$table" "$dbname"
                        fi
                        PS3="$dbname> "
        ...
        2) #-------------------------- Update row data ----------------------#
            if ! file_empty "$table"; then
                ...
                *)
                ...
                #--------------- Field that user want to update part menu -------------------#
