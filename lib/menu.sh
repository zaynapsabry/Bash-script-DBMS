...
function display_create_table_menu {
    ...
    for ((i = 0; i < num_col; i++)); do
        ...
        while [ $valid_choice -eq 1 ]; do
            ...
            if display_select_constrain_menu  "$primary_key_selected" "${constraints[@]}"; then
                echo -e "primary key after disp $primary_key_selected"
                valid_choice=0
...
function display_select_constrain_menu {
    ...
    local primary_key_selected=$1
    local -n constraints=$2 2>/dev/null
    select constrain in "primary key" "unique" "not null" "none"; do
        case $REPLY in
            1)
                if [[ $primary_key_selected == true ]]  ; then
                    
                    echo -e "\e[31mWarning:\e[0m Primary key already exists"
